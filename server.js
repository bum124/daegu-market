...
app.post('/api/ai-recommend', async (req, res) => {
  try {
    ...
    const prompt = `
      ...
      사용자가 제목을 대충 적더라도, 첨부된 '사진'을 최우선으로 분석해서 정확한 제품명과 외관 상태(먼지, 기스 등)를 추론해.
      이를 바탕으로 단일 가격이 아닌, 합리적인 '최저 적정가'와 '최고 적정가' 범위를 제안해줘.

      반드시 "category", "minPrice", "maxPrice", "reason" 4개의 키를 가지는 순수 JSON 개체를 반환해.
      - reason에는 사진에서 어떤 점을 분석해서 이 가격이 나왔는지 짧고 친절하게 적어줘. (예: "사진상 에디파이어 MR4 모델로 보이며, 우퍼 쪽에 약간의 사용감이 있어 감가를 적용했습니다.")
